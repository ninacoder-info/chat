...
Checkout a simple [Demo Application](https://github.com/ninacoder-info/chat-demo)
...
```
composer require ninacoder-info/chat
```
...
Add the `NiNaCoder\Chat\Traits\Messageable` trait to any Model you want to participate in Conversations
For example, let's say we want out `Bot` model to chat with other Models:

```php

use Illuminate\Database\Eloquent\Model;
use NiNaCoder\Chat\Traits\Messageable;
...
Listen for `\NiNaCoder\Chat\Eventing\AllParticipantsDeletedMessage` and 

`\NiNaCoder\Chat\Eventing\AllParticipantsClearedConversation`
